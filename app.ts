...
function filterBtcPairSymbols(symbols: any) {
...
function getNewSymbol(previousSymbols: any, newSymbols: any) {
    let difference = Object.keys(newSymbols).filter(x => !Object.keys(previousSymbols).includes(x));
    return difference[0];
}

function getSymbolPrice(arr1: any, symbol: string) {
    console.log(arr1[symbol]);
    return arr1[symbol];
}

function calculatePercentPrice(price: number, percent: number) {
